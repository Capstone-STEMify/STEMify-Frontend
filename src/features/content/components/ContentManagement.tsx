...
const contentSchema = z.object({
	contentName: z
		.string()
		.min(5, "Content name must be at least 5 characters long"),
...
export default function ContentManagement() {
	...
	const contentId = contentIdRaw
		? Number(Array.isArray(contentIdRaw) ? contentIdRaw[0] : contentIdRaw)
		: undefined;

	const { data: contentData, isLoading: isContentLoading } =
		useGetContentByIdQuery(contentId as number, {
		...
		>
			<div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
				...
				<div className="space-y-6">
					<SCard
						title="Content File"
						description="Upload the file corresponding to the content type"
						content={
							<div className="p-4">
								<form.AppField
									name="file"
									children={(field) => (
										...
										<field.FileField
											...
											label={`Upload ${currentContentType}`}
										/>
									)}
								/>
							</div>
					...
					<form.AppForm>
						<form.SubmitButton className="w-full rounded-full py-3">
