         ...
         <div className='grid grid-cols-3 gap-6 md:gap-8'>
          {statsData.map((stat, index) => (
            <motion.div key={index} variants={staggerItem} className='flex flex-col items-center text-center'>
              <div className={`inline-flex h-12 w-12 items-center justify-center ${stat.bgColor} mb-3 rounded-full shadow-sm`}>
                <stat.icon className={`h-6 w-6 ${stat.iconColor}`} />
              </div>
              <div className='text-2xl font-bold text-gray-800 mb-1'>{stat.value}</div>
              <div className='text-sm font-semibold text-gray-700 mb-1'>{stat.title}</div>
              {stat.subtitle && <div className='text-xs text-gray-500 leading-tight'>{stat.subtitle}</div>}
            </motion.div>
          ))}
        </div>
      </div>
