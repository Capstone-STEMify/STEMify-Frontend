// app/quiz-analytic/components/quiz-result-popup.tsx
"use client";
...
import { Printer, Download, X as XIcon, CheckCircle2, XCircle, HelpCircle, Layers, Star, Clock, Share } from "lucide-react";
...
const AnswerGridItem = ({ status, number }: { status: Status, number: number }) => {
  ...
  return (
    <div className={`flex items-center justify-between p-2 rounded-md border text-sm font-medium ${statusClasses[status]}`}>
      <span>{number}</span>
      {Icon[status]}
    </div>
  );
      ...
      <DialogContent className="max-w-4xl p-8">
        ...
        <div className="my-6 space-y-2">
          <div className="flex justify-between items-start">
            <h3 className="text-2xl font-bold max-w-md">UI Design Fundamentals & Best Practice</h3>
            <div className="flex items-center gap-6 text-sm">
              <div>
                <span className="text-gray-500">Accuracy</span>
                <div className="flex items-center gap-1.5 font-semibold text-lg mt-2">
                  <ProgressCircle value={85} size={28} className="text-green-500" strokeWidth={3.5} showPercentageText={false}/>
                  <span className="text-green-600">85%</span>
                </div>
              </div>
              <div>
                <span className="text-gray-500">Point</span>
                <p className="font-semibold text-lg mt-2">145</p>
              </div>
              <div>
                <span className="text-gray-500">Answered</span>
                <p className="font-semibold text-lg mt-2">19/20</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4 text-xs text-gray-600 mb-6">
          <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-green-500" />Correct 16 - 72%</span>
          <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-yellow-500" />Half Correct 1 - 3%</span>
          <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-red-500" />Incorrect 2 - 12%</span>
          <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-gray-400" />Skipped 1 - 3%</span>
        </div>
        ...
        <Accordion type="single" collapsible className="w-full">
          {detailedQuestionExamples.map((question, index) => (
            <AccordionItem key={index} value={`item-${index + 1}`}>
              <AccordionTrigger className="font-semibold no-underline hover:no-underline [&[data-state=open]>svg]:rotate-180 text-left">
                <div className="flex justify-between items-center w-full pr-4">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600 font-normal">
                     ...
                     <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> Time 32s</span>
                     <span className="flex items-center gap-1.5"><Star className="h-4 w-4 text-yellow-500" /> 1 point</span>
                  </div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pl-12 text-base space-y-4">
                <p>{question.questionText}</p>

                {question.status === 'incorrect' && (
                  <div className="mt-4 p-4 border-l-4 border-green-500 bg-green-50 rounded-r-md">
