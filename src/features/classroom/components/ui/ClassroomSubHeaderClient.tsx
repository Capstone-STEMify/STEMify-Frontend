...
export default function ClassroomSubHeaderClient({ locale, classroomId, curriculumId, classroomName }: Props) {
        ...
        <div className='relative flex h-16 items-center justify-between'>
          ...
          <div className='flex items-center gap-3'>
            <div className='flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-pink-500'>
              <span className='text-lg font-bold text-white'>{classroomName?.charAt(0).toUpperCase() ?? 'C'}</span>
            </div>
            <div className='flex items-center gap-2'>
              <h2 className='text-lg font-bold text-slate-900'>{classroomName ?? 'Classroom'}</h2>
