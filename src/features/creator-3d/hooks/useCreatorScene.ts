...
export function useCreatorScene() {
  ...
  const generateObjectId = useCallback((type: ComponentType): string => {
  ...
  const exportAssembly = useCallback((metadata: {
    title: string
    description: string
    author: string
  }): AssemblyExport => {
    ...
    return {
      ...
      instances: {
        straws: strawInstances.length > 0 ? [
          {
            templateId: 'green_11_2',
            instances: strawInstances
          }
        ] : [],
        connectors: connectorInstances.length > 0 ? [
          {
            templateId: '3leg_red',
