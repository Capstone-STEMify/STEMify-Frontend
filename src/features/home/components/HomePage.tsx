...
import ThreeDSection from "./3d-section/3DSection";
...
interface GetSectionClasses {
	(
		sectionId: string,
		variant?: keyof typeof ANIMATION_VARIANTS | string,
		duration?: number,
		delay?: number,
	): string;
}
...
const useScrollAnimation = () => {
	...
	const getSectionClasses: GetSectionClasses = (
		sectionId,
		variant,
		duration,
		delay,
		...
		const visibleClass =
			ANIMATION_VARIANTS[
				(variant + "Visible") as keyof typeof ANIMATION_VARIANTS
			] || ANIMATION_VARIANTS.fadeUpVisible;
...
export default function HomePage() {
		...
		const handleScroll = (e: WheelEvent) => {
			if (!heroAnimationComplete) {
				...
				if ("deltaY" in e && typeof e.deltaY === "number") {
					...
					"wheelDelta" in e &&
					typeof (e as any).wheelDelta === "number"
				) {
				...
				heroScrollProgress.current = Math.max(
					0,
					Math.min(1, heroScrollProgress.current),
				);
	...
	const handleAnimationComplete: HandleAnimationCompleteProps = (
		isComplete,
	) => {
	...
	return (
		<div ref={containerRef} className="min-h-screen">
			...
			<div
				ref={exploreRef}
				className={getSectionClasses("explore", "fadeUp", 9000, 1200)}
			>
			...
			<div
				ref={toolsRef}
				className={getSectionClasses("tools", "fadeLeft", 9000, 1400)}
			>
				<ToolsSection />
			</div>
			...
			<div
				ref={benefitsRef}
				className={getSectionClasses("benefits", "scaleUp", 9000, 1600)}
			>
				<BenefitsSection />
			</div>
      ...
      <div>
        <ThreeDSection />
			...
			<div
				ref={statsRef}
				className={getSectionClasses("stats", "slideDown", 9000, 1800)}
			>
				<StatsSection />
			</div>
			...
			<div
				ref={testimonialsRef}
				className={getSectionClasses("testimonials", "fadeRight", 9000, 2000)}
			>
				<TestimonialsSection />
			</div>
			...
			<div
				ref={faqRef}
				className={getSectionClasses("faq", "fadeUp", 9000, 2200)}
			>
				<FAQSection />
			</div>
