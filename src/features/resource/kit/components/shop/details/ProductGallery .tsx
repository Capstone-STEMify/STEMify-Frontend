...
const ProductGallery: React.FC<ProductGalleryProps> = ({ kitImages }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
      {kitImages.length > 0 ? (
        <img src={kitImages[currentImage]} alt='Product' className='h-full w-full object-cover' />
      ) : (
        <LoadingComponent />
      )}
    </motion.div>
...
//   >
//     <div className='relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-50 to-slate-100 shadow-2xl'>
