...
export default function PrintableSectionDetail({ sectionId }: PrintableSectionDetailProps) {
  ...
  const { data: contentData, isLoading, isError } = useSearchContentQuery(
    { sectionId },
    { skip: !sectionId }
  );

  if (isLoading) {
    return <div className="py-4"><LoadingComponent textShow={true} /></div>;
  }

  if (isError || !contentData || contentData.data.items.length === 0) {
    return <div className="text-sm text-gray-500 italic py-2">{t('no_section')}</div>;
  }
